...
function filterKeys(type) {
  let count_type = type ? 'count_' + type : 'count_all';
  return function(d) {
...
let debouncedRequest = _debounce(request, 300, { leading: false });
...
function request(url, params, exactMatch, callback, loaded) {
  ...
  d3_json(url, { signal: controller.signal }).then(function(result) {
...
export default {
  ...
  keys: function(params, callback) {
    let doRequest = params.debounce ? debouncedRequest : request;
    ...
    let url = apibase + 'keys/all?' + utilQsString(params);
    doRequest(url, params, false, callback, function(err, d) {
      if (err) {
        callback(err);
      }
      else {
        let f = filterKeys(params.filter);
        let result = d.data.filter(f).sort(sortKeys).map(valKey);
  ...
  multikeys: function(params, callback) {
    ...
    doRequest(url, params, true, callback, function(err, d) {
      if (err) {
        ...
        let f = filterMultikeys(prefix);
        let result = d.data.filter(f).map(valKey);
        _taginfoCache[url] = result;
        callback(null, result);
      }
    });
  ...
  values: function(params, callback) {
    ...
    let url = apibase + 'key/values?' + utilQsString(params);
    doRequest(url, params, false, callback, function(err, d) {
      if (err) {
        callback(err);
      }
        ...
        let re = /network|taxon|genus|species|brand|grape_letiety|royal_cypher|listed_status|booth|rating|stars|:output|_hours|_times|_ref|manufacturer|country|target|brewery|cai_scale/;
        let allowUpperCase = re.test(params.key);
        let f = filterValues(allowUpperCase);

        let result = d.data.filter(f).map(valKeyDescription);
        _taginfoCache[url] = result;
        callback(null, result);
      }
    });
  ...
  roles: function(params, callback) {
    ...
    let url = apibase + 'relation/roles?' + utilQsString(params);
    doRequest(url, params, true, callback, function(err, d) {
      if (err) {
        callback(err);
      }
      else {
        let f = filterRoles(geometry);
        let result = d.data.filter(f).map(roleKey);
        _taginfoCache[url] = result;
        callback(null, result);
      }
    });
  ...
  docs: function(params, callback) {
    ...
    let url = apibase + path + utilQsString(params);
    doRequest(url, params, true, callback, function(err, d) {
      if (err) {
        callback(err);
      }
