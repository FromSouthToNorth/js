...
function getLoc(attrs) {
  let lon = attrs.lon && attrs.lon.value;
  let lat = attrs.lat && attrs.lat.value;
  return [Number(lon), Number(lat)];
}


function getNodes(obj) {
  let elems = obj.getElementsByTagName('nd');
...
function getTags(obj) {
  let elems = obj.getElementsByTagName('tag');
  let tags = {};
  for (let i = 0, l = elems.length; i < l; i++) {
    let attrs = elems[i].attributes;
...
function getMembers(obj) {
  let elems = obj.getElementsByTagName('member');
  let members = new Array(elems.length);
  for (let i = 0, l = elems.length; i < l; i++) {
    let attrs = elems[i].attributes;
    members[i] = {
...
function parseComments(comments) {
  ...
  for (let i = 0; i < comments.length; i++) {
    ...
    if (comment.nodeName === 'comment') {
      let childNodes = comment.childNodes;
      let parsedComment = {};
...
function parseJSON(payload, callback, options) {
  ...
  function parseChild(child) {
    let parser = jsonparsers[child.type];
...
function parseUserJSON(payload, callback, options) {
  ...
  function parseObj(obj) {
    let uid = obj.user.id && obj.user.id.toString();
    if (options.skipSeen && _userCache.user[uid]) {
      delete _userCache.toLoad[uid];
      return null;
    }
    let user = jsonparsers.user(obj.user, uid);
...
let parsers = {
  ...
  note: function parseNote(obj, uid) {
    ...
    let note = new osmNote(props);
    let item = encodeNoteRtree(note);
  ...
  user: function parseUser(obj, uid) {
    ...
    let changesets = obj.getElementsByTagName('changesets');
    if (changesets && changesets[0] && changesets[0].getAttribute('count')) {
      user.changesets_count = changesets[0].getAttribute('count');
    }

    let blocks = obj.getElementsByTagName('blocks');
    if (blocks && blocks[0]) {
      let received = blocks[0].getElementsByTagName('received');
...
function parseXML(xml, callback, options) {
  ...
  function parseChild(child) {
    let parser = parsers[child.nodeName];
    if (!parser) return null;
...
export default {
  ...
  loadFromAPI: function (path, callback, options) {
    ...
    function done(err, payload) {
      ...
      let isAuthenticated = that.authenticated();
  ...
  loadMultiple: function (ids, callback) {
    ...
    Object.keys(groups).forEach(function (k) {
      ...
      let osmIDs = groups[k].map(function (id) { return osmEntity.id.toOSM(id); });
      let options = { skipSeen: false };
  ...
  status: function (callback) {
    ...
    function done(err, xml) {
      ...
      // update blocklists
      let elements = xml.getElementsByTagName('blacklist');
      let regexes = [];
      for (let i = 0; i < elements.length; i++) {
        let regexString = elements[i].getAttribute('regex');  // needs unencode?
        if (regexString) {
          try {
      ...
      if (_rateLimitError) {
        ...
        let maxWayNodes = waynodes.length && parseInt(waynodes[0].getAttribute('maximum'), 10);
        if (maxWayNodes && isFinite(maxWayNodes)) _maxWayNodes = maxWayNodes;

        let apiStatus = xml.getElementsByTagName('status');
        let val = apiStatus[0].getAttribute('api');
  ...
  loadTileAtLoc: function (loc, callback) {
    ...
    let projection = geoRawMercator().transform({ k: k, x: -offset[0], y: -offset[1] });
    let tiles = tiler.zoomExtent([_tileZoom, _tileZoom]).getTiles(projection);
  ...
  loadNotes: function (projection, noteOptions) {
    ...
    let that = this;
    let path = '/api/0.6/notes?limit=' + noteOptions.limit + '&closed=' + noteOptions.closed + '&bbox=';
  ...
  postNoteCreate: function (note, callback) {
    ...
    let path = '/api/0.6/notes?' + utilQsString({ lon: note.loc[0], lat: note.loc[1], text: comment });
  ...
  switch: function (newOptions) {
    ...
    let oldOptions = utilObjectOmit(oauth.options(), 'access_token');
