...
export function rendererPhotos(context) {
  ...
  let _layerIDs = ['streetside', 'mapillary', 'mapillary-map-features', 'mapillary-signs', 'kartaview'];
  let _allPhotoTypes = ['flat', 'panoramic'];
  let _shownPhotoTypes = _allPhotoTypes.slice();   // shallow copy
  ...
  function showsLayer(id) {
    let layer = context.layers().layer(id);
  ...
  photos.init = function() {
    ...
    if (hash.photo_dates) {
      ...
      let parts = /^(.*)[–_](.*)$/g.exec(hash.photo_dates.trim());
    ...
    if (hash.photo_overlay) {
      ...
      let hashOverlayIDs = hash.photo_overlay.replace(/;/g, ',').split(',');
      hashOverlayIDs.forEach(function(id) {
        if (id === 'openstreetcam') id = 'kartaview'; // legacy alias
        let layer = _layerIDs.indexOf(id) !== -1 && context.layers().layer(id);
    ...
    if (hash.photo) {
      ...
      let photoIds = hash.photo.replace(/;/g, ',').split(',');
      let photoId = photoIds.length && photoIds[0].trim();
      let results = /(.*)\/(.*)/g.exec(photoId);
      if (results && results.length >= 3) {
        let serviceId = results[1];
        if (serviceId === 'openstreetcam') serviceId = 'kartaview'; // legacy alias
        let photoKey = results[2];
        ...
        if (service && service.ensureViewerLoaded) {
          ...
          let layer = _layerIDs.indexOf(serviceId) !== -1 && context.layers().layer(serviceId);
          if (layer && !layer.enabled()) layer.enabled(true);
