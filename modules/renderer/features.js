...
export function rendererFeatures(context) {
  let dispatch = d3_dispatch('change', 'redraw');
  let features = utilRebind({}, dispatch, 'on');
  ...
  features.gatherStats = function (d, resolver, dimensions) {
    ...
    let types = utilArrayGroupBy(d, 'type');
    let entities = [].concat(types.relation || [], types.way || [], types.node || []);
    let currHidden, geometry, matches, i, j;
    ...
    if (!_cache[ent].matches) {
      ...
      for (let i = 0; i < _keys.length; i++) {
        if (_keys[i] === 'others') {
          ...
          if (entity.type === 'way') {
            let parents = features.getParents(entity, resolver, geometry);
            ...
            if ((parents.length === 1 && parents[0].isMultipolygon()) ||
              ...
              //
              let pkey = osmEntity.key(parents[0]);
  ...
  features.isHiddenPreset = function (preset, geometry) {
    ...
    let test = preset.setTags({}, geometry);
  ...
  features.isHiddenFeature = function (entity, resolver, geometry) {
    ...
    let matches = Object.keys(features.getMatches(entity, resolver, geometry));
  ...
  features.isHidden = function (entity, resolver, geometry) {
    ...
    let fn = (geometry === 'vertex' ? features.isHiddenChild : features.isHiddenFeature);
  ...
  features.init = function () {
    ...
    if (storage) {
      let storageDisabled = storage.replace(/;/g, ',').split(',');
      storageDisabled.forEach(features.disable);
    }

    let hash = utilStringQs(window.location.hash);
    if (hash.disable_features) {
      let hashDisabled = hash.disable_features.replace(/;/g, ',').split(',');
  ...
  context.history().on('merge.features', function (newEntities) {
    ...
    let handle = window.requestIdleCallback(function () {
      ...
      let types = utilArrayGroupBy(newEntities, 'type');
      // ensure that getMatches is called on relations before ways
      let entities = [].concat(types.relation || [], types.way || [], types.node || []);
      for (let i = 0; i < entities.length; i++) {
        let geometry = entities[i].geometry(graph);
