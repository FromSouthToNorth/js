...
import { IntervalTasksQueue } from '../util/index.js';
...
export function rendererBackgroundSource(data) {
  let source = Object.assign({}, data);   // shallow copy
  let _offset = [0, 0];
  let _name = source.name;
  ...
  source.hasDescription = function () {
    let id_safe = source.id.replace(/\./g, '<TX_DOT>');
    let descriptionText = localizer.tInfo('imagery.' + id_safe + '.description', { default: escape(_description) }).text;
  ...
  source.area = function () {
    ...
    let area = d3_geoArea({ type: 'MultiPolygon', coordinates: [data.polygon] });
  ...
  source.url = function (coord) {
    ...
    if (source.type === 'wms') {
      let tileToProjectedCoords = (function (x, y, z) {
        let zoomSize = Math.pow(2, z);
        let lon = x / zoomSize * Math.PI * 2 - Math.PI;
        let lat = Math.atan(Math.sinh(Math.PI * (1 - 2 * y / zoomSize)));
        ...
        switch (source.projection) {
          ...
          default: // EPSG:3857 and synonyms
            let mercCoords = d3_geoMercatorRaw(lon, lat);
            return {
      ...
      let maxXminY = tileToProjectedCoords(coord[0] + 1, coord[1] + 1, coord[2]);
...
rendererBackgroundSource.Bing = function (data, dispatch) {
  ...
  let key = utilAesDecrypt('5c875730b09c6b422433e807e1ff060b6536c791dbfffcffc4c6b18a1bdba1f14593d151adb50e19e1be1ab19aef813bf135d0f103475e5c724dec94389e45d0');
  /*
  ...
  */
  ...
  let url = 'https://dev.virtualearth.net/REST/v1/Imagery/Metadata/AerialOSM?include=ImageryProviders&uriScheme=https&key=' + key;
  let cache = {};
  let inflight = {};
  let providers = [];
  ...
  bing.getMetadata = function (center, tileCoord, callback) {
    ...
    let url = 'https://dev.virtualearth.net/REST/v1/Imagery/BasicMetadata/AerialOSM/' + centerPoint +
      '?zl=' + zoom + '&key=' + key;
...
rendererBackgroundSource.Esri = function (data) {
  ...
  let esri = rendererBackgroundSource(data);
  let cache = {};
  let inflight = {};
  let _prevCenter;
  ...
  esri.fetchTilemap = function (center) {
    ...
    let dummyUrl = esri.url([1, 2, 3]);

    // calculate url z/y/x from the lat/long of the center of the map
    let x = (Math.floor((center[0] + 180) / 360 * Math.pow(2, z)));
    let y = (Math.floor((1 - Math.log(Math.tan(center[1] * Math.PI / 180) + 1 / Math.cos(center[1] * Math.PI / 180)) / Math.PI) / 2 * Math.pow(2, z)));
...
rendererBackgroundSource.None = function () {
  let source = rendererBackgroundSource({ id: 'none', template: '' });
...
rendererBackgroundSource.Custom = function (template) {
  let source = rendererBackgroundSource({ id: 'custom', template: template });
