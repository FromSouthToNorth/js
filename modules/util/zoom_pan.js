...
export function utilZoomPan() {
    ...
    zoom.transform(selection, function() {
      let e = extent.apply(this, arguments),
        ...
        p0 = !p ?
          ...
          typeof p === 'function' ? p.apply(this, arguments) : p;
      return constrain(
        d3_zoomIdentity.translate(p0[0], p0[1])
          .scale(t.k)
          .translate(
            typeof x === 'function' ? -x.apply(this, arguments) : -x,
            typeof y === 'function' ? -y.apply(this, arguments) : -y,
  ...
  function pointerdown(d3_event) {
    ...
    let g = gesture(this, arguments, _downPointerIDs.size === 1);
    let started;

    d3_event.stopImmediatePropagation();
    _pointerLocGetter = utilFastMouse(this);
    let loc = _pointerLocGetter(d3_event);
    let p = [loc, _transform.invert(loc), d3_event.pointerId];
    if (!g.pointer0) {
      g.pointer0 = p;
      started = true;
