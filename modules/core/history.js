...
export function coreHistory(context) {
  let dispatch = d3_dispatch('reset', 'change', 'merge', 'restore', 'undone', 'redone', 'storage_error');
  ...
  let history = {
    ...
    merge: function (entities/*, extent*/) {
      let stack = _stack.map(function (state) { return state.graph; });
    ...
    toIntroGraph: function () {
      ...
      function copyIntroEntity(source) {
        let copy = utilObjectOmit(source, ['type', 'user', 'v', 'version', 'visible']);
    ...
    fromJSON: function (json, loadChildNodes) {
      ...
      if (h.version === 2 || h.version === 3) {
        ...
        if (h.version === 3) {
          ...
          let baseEntities = h.baseEntities.map(function (d) { return osmEntity(d); });
          let stack = _stack.map(function (state) { return state.graph; });
          ...
          if (loadChildNodes) {
            ...
            let baseWays = baseEntities
            .filter(function (e) { return e.type === 'way'; });
            let nodeIDs = baseWays
            ...
            if (missing.length && osm) {
              ...
              let childNodesLoaded = function (err, result) {
                if (!err) {
                  ...
                  if (visibles.length) {
                    let visibleIDs = visibles.map(function (entity) { return entity.id; });
                    let stack = _stack.map(function (state) { return state.graph; });
