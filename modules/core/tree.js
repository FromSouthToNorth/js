...
export function coreTree(head) {
  ...
  let _segmentsRTree = new RBush();
  let _segmentsBBoxes = {};
  let _segmentsByWayId = {};

  let tree = {};

  function entityBBox(entity) {
    let bbox = entity.extent(head)
      .bbox();
    bbox.id = entity.id;
  ...
  function loadEntities(entities) {
    ...
    if (segments.length) _segmentsRTree.load(
      segments.map(segmentBBox)
        .filter(Boolean));
  }

  function updateParents(entity, insertions, memo) {
    head.parentWays(entity)
  ...
  function updateToGraph(graph) {
    ...
    let diff = coreDifference(head, graph);

    head = graph;

    let changed = diff.didChange;
