...
export function coreValidator(context) {
  ...
  function parseHashParam(param) {
    ...
    function makeRegExp(str) {
      const escaped = str.replace(/[-\/\\^$+?.()|[\]{}]/g,
          '\\$&')   // escape all reserved chars except for the '*'
  ...
  function revalidateUnsquare(cache) {
    ...
    const buildings = context.history().
        tree().
        intersects(geoExtent([-180, -90], [180, 90]),
            cache.graph)  // everywhere
  ...
  validator.getIssues = (options) => {
    ...
    if (_headCache.graph && _headCache.graph !== _baseCache.graph) {
      Object.values(_headCache.issuesByIssueID).forEach(issue => {
        ...
        const userModified = (issue.entityIds || []).some(
            id => _completeDiff.hasOwnProperty(id));
    ...
    function filter(issue) {
      ...
      if (opts.includeDisabledRules === 'only' &&
          !_disabledRules[issue.type]) return false;
      if (!opts.includeDisabledRules &&
          _disabledRules[issue.type]) return false;

      if (opts.includeIgnored === 'only' &&
          !_ignoredIssueIDs.has(issue.id)) return false;
      ...
      if ((issue.entityIds || []).some(
          id => !context.hasEntity(id))) return false;
  ...
  validator.getSharedEntityIssues = (entityIDs, options) => {
    ...
    return allIssues.filter(issue => (issue.entityIds || []).some(
        entityID => forEntityIDs.has(entityID))).sort((issue1, issue2) => {
    ...
    function runValidation(key) {
      ...
      function applySeverityOverrides(issue) {
        ...
        for (i = 0; i < _errorOverrides.length; i++) {
          if (_errorOverrides[i].type.test(type) &&
        ...
        for (i = 0; i < _warningOverrides.length; i++) {
          if (_warningOverrides[i].type.test(type) &&
        ...
        for (i = 0; i < _disableOverrides.length; i++) {
          if (_disableOverrides[i].type.test(type) &&
