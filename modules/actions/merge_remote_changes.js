...
export function actionMergeRemoteChanges(
  ...
  function mergeNodes(base, remote, target) {
    ...
    let b = remote.nodes || [];
    let nodes = [];
    let hunks = diff3Merge(a, o, b, { excludeFalseConflicts: true });
  ...
  function mergeChildren(targetWay, children, updates, graph) {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      let local = localGraph.hasEntity(id);
      let remote = remoteGraph.hasEntity(id);
      let target;
  ...
  let action = function(graph) {
    let updates = { replacements: [], removeIds: [] };
    let base = graph.base().entities[id];
