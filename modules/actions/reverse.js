...
export function actionReverse(entityID, options) {
  let ignoreKey = /^.*(_|:)?(description|name|note|website|ref|source|comment|watch|attribution)(_|:)?/;
  let numeric = /^([+\-]?)(?=[\d.])/;
  let directionKey = /direction$/;
  let turn_lanes = /^turn:lanes:?/;
  let keyReplacements = [
    [/:right$/, ':left'],
    [/:left$/, ':right'],
    [/:forward$/, ':backward'],
  ...
  let onewayReplacements = {
    yes: '-1',
    '1': '-1',
    '-1': 'yes',
  };

  let compassReplacements = {
    N: 'S',
    NNE: 'SSW',
    NE: 'SW',
    ENE: 'WSW',
  ...
  function reverseNodeTags(graph, nodeIDs) {
    for (let i = 0; i < nodeIDs.length; i++) {
      let node = graph.hasEntity(nodeIDs[i]);
