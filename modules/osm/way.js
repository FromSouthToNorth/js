...
Object.assign(osmWay.prototype, {
  ...
  impliedLineWidthMeters: function () {
    ...
    for (let key in averageWidths) {
      if (this.tags[key] && averageWidths[key][this.tags[key]]) {
        let width = averageWidths[key][this.tags[key]];
        if (key === 'highway') {
  ...
  isConvex: function (resolver) {
    ...
    for (let i = 0; i < coords.length; i++) {
      let o = coords[(i + 1) % coords.length];
      let a = coords[i];
      let b = coords[(i + 2) % coords.length];
      let res = geoVecCross(a, b, o);
  ...
  segments: function (graph) {
    ...
    function segmentExtent(graph) {
      ...
      let n2 = graph.hasEntity(this.nodes[1]);
