...
export function osmOldMultipolygonOuterMemberOfRelation(entity, graph) {
  ...
  let outerMember;
  for (let memberIndex in entity.members) {
    let member = entity.members[memberIndex];
...
export function osmIsOldMultipolygonOuterMember(entity, graph) {
  ...
  let parents = graph.parentRelations(entity);
  ...
  let members = parent.members, member;
...
export function osmOldMultipolygonOuterMember(entity, graph) {
  ...
  let members = parent.members, member, outerMember;
  for (let i = 0; i < members.length; i++) {
    member = members[i];
    if (!member.role || member.role === 'outer') {
...
export function osmJoinWays(toJoin, graph) {
  ...
  function reverse(item) {
    let action = actionReverse(item.id, { reverseOneway: true });
  ...
  while (toJoin.length) {
    ...
    let currWays = [item];
    let currNodes = resolve(item).slice();
