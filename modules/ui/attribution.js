...
export function uiAttribution(context) {
  ...
  function render(selection, data, klass) {
    ...
    attributions = attributions.enter()
      ...
      .each((d, i, nodes) => {
        ...
        if (d.terms_url) {
          attribution = attribution.append('a')
    ...
    let copyright = attributions.selectAll('.copyright-notice')
      .data(d => {
        let notice = d.copyrightNotices(context.map()
            .zoom(),
          context.map()
            .extent());
  ...
  function update() {
    let baselayer = context.background()
      .baseLayerSource();
    _selection.call(render, (baselayer ? [baselayer] : []),
      'base-layer-attribution');

    const z = context.map()
      .zoom();
    let overlays = context.background()
      .overlayLayerSources() || [];
    _selection.call(render, overlays.filter(s => s.validZoom(z)),
