...
export function uiSectionBackgroundList(context) {
  ...
  let _customSource = context.background().findSource('custom');
  ...
  function renderDisclosureContent(selection) {
    ...
    // the background list
    let container = selection.selectAll('.layer-background-list').data([0]);
  ...
  function setTooltips(selection) {
    selection.each(function(d, i, nodes) {
      let item = d3_select(this).select('label');
  ...
  function drawListItems(layerList, type, change, filter) {
    ...
    label.append('span').each(function(d) { d.label()(d3_select(this)); });
  ...
  function chooseBackground(d) {
    ...
    let previousBackground = context.background().baseLayerSource();
  ...
  context.map().on('move.background_list',
      _debounce(function() {
