...
export function uiMapInMap(context) {
  ...
  function mapInMap(selection) {
    ...
    let dataLayer = svgData(projection, context)
      .showLabels(false);
    ...
    function zoomed(d3_event) {
      ...
      _isTransformed = true;
      _tCurr = d3_zoomIdentity.translate(x, y)
        .scale(k);
    ...
    function zoomEnded() {
      ...
      _gesture = null;
      context.map()
        .center(projection.invert(_cMini));   // recenter main map..
    }
    ...
    function redraw() {
      ...
      // redraw overlay
      let overlaySources = context.background()
        .overlayLayerSources();
