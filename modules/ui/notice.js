...
export function uiNotice(context) {
  ...
  return function(selection) {
    let div = selection.append('div').attr('class', 'notice');
    ...
    let button = div.append('button').
        ...
        on('wheel', function(d3_event) {   // let wheel events pass through #4482
          let e2 = new WheelEvent(d3_event.type, d3_event);
          context.surface().node().dispatchEvent(e2);
        });

    button.call(svgIcon('#iD-icon-plus', 'pre-text')).
        append('span').
        attr('class', 'label').
        call(t.append('zoom_in_edit'));

    function disableTooHigh() {
      let canEdit = context.map().zoom() >= context.minEditableZoom();
