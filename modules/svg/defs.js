...
export function svgDefs(context) {
  ...
  function drawDefs(selection) {
    ...
    // add patterns
    let patterns = _defsSelection.selectAll('pattern')
      .data([
  ...
  function addSprites(ids, overrideColors) {
    ...
    let spritesheets = _defsSelection.selectAll('.spritesheet')
    ...
    spritesheets.enter()
      ...
      .each(function(d) {
        let url = context.imagePath(d + '.svg');
        let node = d3_select(this)
          .node();
