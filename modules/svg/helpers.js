    ...
    d3_geoStream({
      ...
      point: function(x, y) {
        ...
        if (a) {
          ...
          if (span >= 0) {
            let heading = geoVecAngle(a, b);
            let dx = dt * Math.cos(heading);
...
export function svgSegmentWay(way, graph, activeID) {
  ...
  function getWaySegments() {
    let isActiveWay = (way.nodes.indexOf(activeID) !== -1);
    let features = { passive: [], active: [] };
    let start = {};
    let end = {};
    let node, type;
